...
class Frontend(ABC):
    ...
    LEGACY_REQUIRES: tuple[Requirement, ...] = (
    ...
    def prepare_metadata_for_build_wheel(
        ...
        if basename is None:
            ...
            basename, err, out = self._metadata_from_built_wheel(
                config_settings, metadata_directory, "build_wheel"
            )
    ...
    def _check_metadata_dir(self, metadata_directory: Path) -> None:
        if metadata_directory == self._root:
            raise RuntimeError(
                f"the project root and the metadata directory can't be the same {self._root}"
            )
    ...
    def prepare_metadata_for_build_editable(
        ...
        if basename is None:
            # if backend does not provide it acquire it from the wheel
            basename, err, out = self._metadata_from_built_wheel(
                config_settings, metadata_directory, "build_editable"
            )
        if not isinstance(basename, str):
            self._unexpected_response(
                "prepare_metadata_for_build_wheel", basename, str, out, err
    ...
    def _metadata_from_built_wheel(
        ...
        with self._wheel_directory() as wheel_directory:
            ...
            if basename is None:  # pragma: no branch
                raise RuntimeError(
                    f"no .dist-info found inside generated wheel {wheel}"
                )
