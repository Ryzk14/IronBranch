...
class Pep517VirtualEnvPackager(PythonPackageToxEnv, VirtualEnv):
    ...
    def perform_packaging(self, for_env: EnvConfigSet) -> list[Package]:
        ...
        if of_type == "editable-legacy":
            ...
            deps = [
                *self.requires(),
                *self._frontend.get_requires_for_build_sdist().requires,
            ] + deps
    ...
    def _ensure_meta_present(self, for_env: EnvConfigSet) -> None:
        ...
        if for_env["package"] == "editable":
            dist_info = end.prepare_metadata_for_build_editable(
                self.meta_folder, self._wheel_config_settings
            ).metadata
        else:
            dist_info = end.prepare_metadata_for_build_wheel(
                self.meta_folder, self._wheel_config_settings
            ).metadata
...
class Pep517VirtualEnvFrontend(Frontend):
    def __init__(self, root: Path, env: Pep517VirtualEnvPackager) -> None:
        ...
        pkg_cache = cached(
            into,
            key=lambda *args, **kwargs: "wheel"
            if "wheel_directory" in kwargs
            else "sdist",  # noqa: U100
        )
    ...
    def _send(self, cmd: str, **kwargs: Any) -> tuple[Any, str, str]:
        try:
            if cmd in (
                ...
                if (
                    "wheel" in self._tox_env.builds
                    or "editable" in self._tox_env.builds
                ):
                    return (
                        None,
                        "",
    ...
    def _unexpected_response(
        self, cmd: str, got: Any, expected_type: Any, out: str, err: str
    ) -> NoReturn:
        try:
