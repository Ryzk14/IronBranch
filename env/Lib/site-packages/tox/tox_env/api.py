...
class ToxEnv(ABC):
    ...
    def register_config(self) -> None:
        ...
        )  # trigger installer creation to allow configuration registration
    ...
    def _platform_check(self) -> None:
        ...
        if platform_str:
            ...
            if match is None:
                raise Skip(
                    f"platform {self.runs_on_platform} does not match {platform_str}"
                )
    ...
    def execute_async(
        ...
        if show and self._hidden_outcomes is not None:
            if (
                execute_status.outcome is not None
            ):  # pragma: no cover # if it gets cancelled before even starting
    ...
    def display_context(self, suspend: bool) -> Iterator[None]:
        with self._log_context():
            with self.log_handler.suspend_out_err(
