...
def replace(
    conf: Config, loader: IniLoader, value: str, args: ConfigLoadArgs, depth: int = 0
) -> str:
    """Replace all active tokens within value according to the config."""
    if depth > MAX_REPLACE_DEPTH:
        raise MatchRecursionError(
            f"Could not expand {value} after recursing {depth} frames"
        )
    return Replacer(conf, loader, conf_args=args, depth=depth).join(
...
class Replacer:
    ...
    def __init__(
        self, conf: Config, loader: IniLoader, conf_args: ConfigLoadArgs, depth: int = 0
    ):
        self.conf = conf
        self.loader = loader
    ...
    def _replace_match(self, value: MatchExpression) -> str:
        ...
        if replace_value is not None:
            ...
            if needs_expansion:
                try:
                    return replace(
                        self.conf, self.loader, replace_value, conf_args, self.depth + 1
                    )
...
def replace_reference(
    conf: Config, loader: IniLoader, value: str, conf_args: ConfigLoadArgs
) -> str | None:
    ...
    if match:
        ...
        try:
            for src in _config_value_sources(
                settings["env"], settings["section"], conf_args.env_name, conf, loader
            ):
                try:
...
def replace_pos_args(conf: Config, args: list[str], conf_args: ConfigLoadArgs) -> str:
    ...
    if pos_args is None:
        replace_value = ARG_DELIMITER.join(
            args
        )  # if we use the defaults join back remaining args
    else:
